/**
 * v5_iid_debug.c — v4 + 内部迭代加深 (Internal Iterative Deepening)
 *
 * 新增改动：
 *   + TT 未命中（无 best_move 提示）且 depth >= 5 时，
 *     先以 depth/2 做浅搜索，获得走法排序依据
 *   + 代价是一次浅搜索，收益是第一个走法就很可能是最好的 → PVS 效率翻倍
 *
 * 效果：TT 冷启动时减少全窗口重搜索次数
 */
