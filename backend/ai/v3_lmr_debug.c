/**
 * v3_lmr_debug.c — v2 + 延迟走法缩减 (Late Move Reductions)
 *
 * 新增改动：
 *   + moves_searched 计数器，跟踪已搜索的走法数
 *   + 搜索 >= 4 个走法后、depth >= 3 时，对靠后的走法应用 LMR
 *   + 缩减量 = 1 + (moves_searched - 4) / 8，上限 3
 *   + 缩减搜索通过零窗口 → 全深度再搜；不通过则跳过全深度
 *
 * 效果：排序靠后的走法大概率是坏棋，用缩减深度快速过滤
 */
...
typedef struct {
  ...
  long long lmr_attempts;      /* ★ v3 生效 */
  long long lmr_researches;    /* ★ v3 生效 */
...
void tt_store(uint64 key, int depth, int score, int alpha, int beta, int best_move) {
  ...
  if (entry->key == 0 || depth >= entry->depth || entry->key != key) {
    ...
    if (score <= alpha) flag = TT_ALPHA; else if (score >= beta) flag = TT_BETA;
  ...
  /* 剩余走法 */
  {
    ...
    for (int idx = 0; idx < size; idx++) {
      ...
      if (is_first_move) {
        ...
        score = -negamax(depth - 1 - lmr_reduction, nO, nP, -alpha - 1, -alpha, NULL, true);
        if (score > alpha) {
