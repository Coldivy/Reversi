/**
 * v2_nullmove_debug.c — v1 + 空步裁剪 (Null Move Pruning)
 *
 * 新增改动：
 *   + negamax 增加 allow_null 参数，防止连续空步误判迫移局面
 *   + depth >= 4 且空格 > 12 时，尝试「轮空」一回合
 *     若对手在缩减深度下仍无法超过 beta → 直接截断
 *   + R = 3 + depth/4，null_depth 守卫 >= 1
 *
 * 效果：已明显占优的节点被大幅跳过
 */
...
typedef struct {
  ...
  long long tt_hits;
  long long beta_cuts;
  long long null_cuts;         /* ★ v2 生效 */
  long long futility_skips;    /* 恒为 0 */
  long long lmr_attempts;      /* 恒为 0 */
  long long lmr_researches;    /* 恒为 0 */
  long long iid_searches;      /* 恒为 0 */
...
typedef struct {
  uint64 key; int score; int best_move; int depth; uint8_t flag;
} TTEntry;
...
#define DIRECTION(SHIFT, MASK) \
  t = (P SHIFT) & (MASK); for (int _i = 0; _i < 5; ++_i) t |= (t SHIFT) & (MASK); moves |= (t SHIFT);
...
#define FLIP_DIR(SHIFT, MASK) \
  { uint64 t = (m SHIFT) & (MASK); for (int _i = 0; _i < 5; ++_i) t |= (t SHIFT) & (MASK); if ((t SHIFT) & P) flipped |= t; }
...

  {
    ...
    for (int idx = 0; idx < size; idx++) {
      ...
      if (is_first_move) {
        ...
        score = -negamax(depth - 1, nO, nP, -alpha - 1, -alpha, NULL, true);
      ...
      if (alpha >= beta) {
        if (i != tt_move) {
          if (killer_moves[depth][0] != i) {
            killer_moves[depth][1] = killer_moves[depth][0]; killer_moves[depth][0] = i;
          }
        }
...
int solve(uint64 P, uint64 O, int alpha, int beta, int *best_move) {
  STATS_INC(nodes);
  int occupied = popcount(P | O), depth = 64 - occupied, original_alpha = alpha;
  uint64 key = get_zobrist_key(P, O);
  int tt_move = -1, tt_score = -1;
...
int c_get_best_move(int depth, uint64 player_bb, uint64 opponent_bb, int *move) {
  ...
  for (int d = 1; d <= depth; d++) {
    ...
    if (d >= 3) {
      int margin = 50, alpha = score - margin, beta = score + margin;
      while (1) {
        if (alpha < LOSS_SCORE) alpha = LOSS_SCORE;
        if (beta  > WIN_SCORE)  beta  = WIN_SCORE;
        current_best_move = -1;
        int val = negamax(d, player_bb, opponent_bb, alpha, beta, &current_best_move, true);  /* ★ allow_null=true */
        if (val <= alpha) {
      ...
      score = negamax(d, player_bb, opponent_bb, LOSS_SCORE, WIN_SCORE, &current_best_move, true);
    }
