/**
 * v4_futility_debug.c — v3 + 静态剪枝 (Futility Pruning)
 *
 * 新增改动：
 *   + depth==1 且已搜索 >= 3 个走法时，走完该步的 stand_pat = -evaluate(nO, nP)
 *     若 stand_pat + 300 < alpha，跳过此走法（太差不可能成为最佳）
 *   + 保留 stand_pat 作为 best_score 的底线
 *
 * 效果：horizon=1 的叶子层大幅减少无谓的评估搜索
 */
...
typedef struct {
  ...
  long long null_cuts;   long long futility_skips;  /* ★ v4 生效 */
