/**
 * v7_nonull_debug.c — v6 去掉空步裁剪版本
 *
 * 包含所有 v6 的优化，唯一切除的是空步裁剪 (Null Move Pruning)。
 *
 * 原因：黑白棋中盘常处于「迫移」(zugzwang) 状态——
 * 「多走一步反而是劣势，落子=给对手暴露新前沿，甚至送角」。
 * 空步裁剪基于象棋假设「轮空=损失」，在黑白棋中盘可能错误截断。
 *
 * 保留的优化：
 *   ✓ 历史启发表
 *   ✓ 延迟走法缩减 (LMR)
...
/* ================================================================== */
/*  调试计数器                                                        */
/*  注意：null_cuts 恒为 0，因为空步裁剪已切除                       */
/* ================================================================== */

typedef struct {
  long long nodes;       long long tt_hits;     long long beta_cuts;
  long long null_cuts;   /* ✗ 恒为 0 */
  long long futility_skips;
...
int negamax(int depth, uint64 P, uint64 O,
   ...
   *  ✗ 空步裁剪已切除
   *
   *  原因：黑白棋中盘处于迫移状态时，「多走一步=劣势」。
   *  空步裁剪假设「轮空对己方不利」，这在黑白棋中常常是错的。
   *  错误截断会漏掉那些「宁可跳步让对手暴露弱点」的好棋。
   * ================================================================ */
  ...
  /* 剩余走法：排序 + PVS + LMR + 静态剪枝 */
  {
    ...
    for (int idx = 0; idx < size; idx++) {
      ...
      /* PVS + LMR */
      if (is_first_move) { score = -negamax(depth - 1, nO, nP, -beta, -alpha, NULL); is_first_move = false; }
      else if (lmr_reduction > 0) {
        STATS_INC(lmr_attempts);
        score = -negamax(depth - 1 - lmr_reduction, nO, nP, -alpha - 1, -alpha, NULL);
        if (score > alpha) { STATS_INC(lmr_researches); score = -negamax(depth - 1, nO, nP, -alpha - 1, -alpha, NULL); }
      } else { score = -negamax(depth - 1, nO, nP, -alpha - 1, -alpha, NULL); }
...
int solve(uint64 P, uint64 O, int alpha, int beta, int *best_move) {
  ...
  /* 启发式排序 */
  {
    ...
    while (temp_moves) {
      ...
      if (idx == killer_moves[depth][0]) sc += 20000; else if (idx == killer_moves[depth][1]) sc += 10000;
      int j = size; while (j > 0 && rated_moves[j - 1].score < sc) { rated_moves[j] = rated_moves[j - 1]; j--; }
      rated_moves[j] = (RatedMove){.move = idx, .score = sc}; size++; temp_moves &= temp_moves - 1;
    }
    for (int idx = 0; idx < size; idx++) {
      int i = rated_moves[idx].move; uint64 nP, nO; make_move(P, O, i, &nP, &nO);
      int score = -solve(nO, nP, -beta, -alpha, NULL);
      if (score > best_score) { best_score = score; local_best_move = i; }
      if (score > alpha) alpha = score;
  ...
  for (int d = 1; d <= depth; d++) {
    ...
    if (d >= 3) {
      ...
      while (1) {
        ...
        current_best_move = -1;
        int val = negamax(d, player_bb, opponent_bb, a, b, &current_best_move);
    ...
    } else { score = negamax(d, player_bb, opponent_bb, LOSS_SCORE, WIN_SCORE, &current_best_move); }
