/**
 * v1_history_debug.c — v0 + 历史启发表 (History Heuristic)
 *
 * 改动：
 *   + history_table[64]：每个格子引发 Beta 截断时累加 depth*depth
 *   + 走法排序：静态权重 + 历史值/16 + 杀手加分
 *   + 防溢出：任意格 > HISTORY_MAX 时全体折半
 *
 * 效果：走法排序精度提升 → Beta 截断更早到来 → 节点数减少
 */
...
static int history_table[64];   /* ★ v1 新增 */
...
static inline int move_ordering_score(int move, int depth) {
  ...
  score += history_table[move] / 16;          /* ★ 历史启发 */
